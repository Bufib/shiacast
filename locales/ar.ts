const ar = {
  // LanguageSelectionScreen
  chooseLanguage: "اختر لغتك",

  // Videos
  videosTitle: "الفيديوهات",
  videosEmpty: "لا توجد فيديوهات حتى الآن",
  watched: "تمت المشاهدة",
  finished: "مكتمل",
  marked_as_watched: "تم وضع علامة: تمت المشاهدة",
  unmarked_as_watched: "تمت إزالة علامة المشاهدة",
  marked_as_finished: "تم وضع علامة: مكتمل",
  unmarked_as_finished: "تمت إزالة علامة الاكتمال",
  videoUnavailable: "الفيديو غير متاح",
  watchOnYoutube: "شاهد على يوتيوب",
  playVideo: "تشغيل الفيديو",
  all: "الكل",
  allTopics: "كل المواضيع",
  allAuthors: "كل المؤلفين",
  allLanguages: "كل اللغات",
  topics: "المواضيع",
  uncategorizedTopic: "بدون موضوع",
  authors: "المؤلفون",
  filter: "تصفية",
  resetFilters: "إعادة تعيين التصفية",

  // General
  today: "اليوم",
  yes: "نعم",
  reset: "إعادة تعيين",
  retry: "إعادة المحاولة",
  errorLoadingData: "حدث خطأ أثناء تحميل البيانات",
  openingLink: "جارٍ فتح الرابط",
  urlNotSupported: "الرابط غير مدعوم",
  errorOpeningUrl: "حدث خطأ أثناء فتح الرابط",
  back: "رجوع",
  small: "صغير",
  medium: "متوسط",
  large: "كبير",
  error: "خطأ",
  errorTitle: "خطأ",
  successTitle: "نجاح",
  favorites: "المفضلة",
  search: "بحث",
  noSearchResult: "لم يتم العثور على فيديوهات مطابقة",
  remove: "إزالة",
  delete: "حذف",
  cancel: "إلغاء",
  create: "إنشاء",
  pickColor: "اختر لونًا",
  noFavorites: "ليس لديك أي عناصر مفضلة بعد!",
  noFoldersYet: "ليس لديك أي مجلدات بعد",
  enterFolderName: "أدخل اسمًا للمجلد",
  done: "تم",
  confirm: "تأكيد",
  noData: "لا توجد بيانات",
  feedback: "ملاحظات",
  loading: "جارٍ تحميل البيانات",

  // Settings
  darkMode: "الوضع الداكن",
  enableDarkMode: "تفعيل الوضع الداكن",
  notifications: "الإشعارات",
  receivePushNotifications: "استلام الإشعارات",
  clearAppCache: "مسح ذاكرة التخزين المؤقت",
  clearAppCacheConfirmTitle: "مسح ذاكرة التخزين المؤقت",
  clearAppCacheConfirmMessage:
    "هل تريد حقًا حذف جميع الملفات المخزنة مؤقتًا في التطبيق؟",
  clearAppCacheSuccessMessage:
    "تم مسح ذاكرة التخزين المؤقت للتطبيق بنجاح.",
  dataPrivacy: "حماية البيانات",
  aboutTheApp: "حول التطبيق",
  imprint: "البيانات القانونية",
  appVersion: "إصدار التطبيق",

  // Navigation
  settings: "الإعدادات",
  home: "الرئيسية",

  // Force update
  updateRequiredTitle: "التحديث مطلوب",
  updateRequiredMessage:
    "يتوفر إصدار جديد من التطبيق. يرجى تحديث التطبيق للمتابعة.",
  updateNow: "حدّث الآن",

  // Push-notification
  pushNotificationsDisabledTitle: "الإشعارات معطلة",
  pushNotificationsDisabledMessage:
    "لتلقي الإشعارات، يرجى تفعيلها من الإعدادات.",
  openSettings: "فتح الإعدادات",
  noInternetConnectionTitle: "لا يوجد اتصال بالإنترنت",
  noInternetConnectionMessage: "يرجى التحقق من اتصالك.",
  ok: "حسنًا",

  // Language selector
  language: "اللغة",
  selectAppLanguage: "اختر لغة التطبيق",
  deutsch: "Deutsch",
  english: "English",
  arabic: "العربية",

  // About
  aboutAppContent:
    "شيعكاست هو مجموعة من الفيديوهات الإسلامية بعدة لغات. يتم توفير المحتوى عبر يوتيوب ويتم تجميعه بشكل مريح داخل التطبيق – مع إمكانية التصفية حسب اللغة والموضوع والمؤلف، بالإضافة إلى مجلدات المفضلة الشخصية.",
} as const;

export default ar;
