...
/**
 * Get all prayer recommendations for a given language.
 */
export async function getPrayerRecommendations(
  languageCode: string,
): Promise<PrayerRecommendationType[]> {
  const db = getDatabase();
  return db.getAllAsync<PrayerRecommendationType>(
