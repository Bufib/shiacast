...
function VerseCard({
  ...
  const fontSizeLatin = getScaledFontSize(baseFontSize, "latin");
  const lineHeightLatin = getScaledLineHeight(baseFontSize, "latin");

  const fontSizeTranslit = getScaledFontSize(baseFontSize, "transliteration");
  const lineHeightTranslit = getScaledLineHeight(
    baseFontSize,
    "transliteration",
  );

  const fontSizeArabic = getScaledFontSize(baseFontSize, "arabic");
  const lineHeightArabic = getScaledLineHeight(baseFontSize, "arabic");
  ...
  return (
    ...
    >
      ...
      <View style={styles.content}>
        ...
        {!!transliterationText && (
          <View
            style={styles.transliterationWrap}
            onLayout={(e) => {
              const w = e.nativeEvent.layout.width;
              if (w > 0 && w !== htmlWidth) setHtmlWidth(w);
            }}
