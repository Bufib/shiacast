...
export function AddRecommendedActsModal({ visible, onClose, acts }: Props) {
  ...
  const handleAdd = useCallback(() => {
    const daysToAdd = allDays ? DAY_INDICES : [selectedDay];
    for (const dayIndex of daysToAdd) {
      const existingTexts = new Set(
        (todosByDay[dayIndex] ?? []).map((item) =>
          item.text.trim().toLowerCase(),
        ),
      );
      for (const act of selectedActs) {
        if (!existingTexts.has(act.trim().toLowerCase())) {
          addTodo(dayIndex, act);
        }
      }
    }
    onClose();
  }, [selectedActs, selectedDay, allDays, todosByDay, addTodo, onClose]);
        ...
        <View style={[styles.sheet, { backgroundColor: bgColor }]}>
          ...
          >
            ...
            >
              ...
              <TouchableOpacity
                style={[{}]}
                onPress={() => setAllDays((prev) => !prev)}
                activeOpacity={0.7}
              >
                <ThemedText
                  style={[
                    styles.dayChipText,
                    allDays && styles.dayChipTextActive,
                    {
                      fontSize: 14,
                      color: allDays
                        ? Colors.universal.primary
            ...
            >
              {DAY_INDICES.map((dayIndex) => {
                const isActive = allDays || selectedDay === dayIndex;
                return (
                  <TouchableOpacity
                    key={dayIndex}
                    style={[
