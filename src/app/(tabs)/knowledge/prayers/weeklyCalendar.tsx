...
//       <Stack.Screen
//         options={{
//           title: t("weeklyToDoTitle"),
//           headerRight: () => {
//             return (
//               <TouchableOpacity style={{backgroundColor: Colors.universal.third, padding: 5, borderRadius: 20}} onPress={()=> router.push("/knowledge/prayers/prayerRecommendations")}>
//                 <ThemedText style={{color: Colors.universal.favorite}}>{t("showPrayerRecommendation")}</ThemedText>
//               </TouchableOpacity>
//             );
//           },
//         }}
...
import { useTranslation } from "react-i18next";
import { TodoToDeleteType } from "@/constants/Types";
import { getPrayerRecommendations } from "../../../../../db/queries/prayers";
import {
  cancelTodoReminderNotification,
  scheduleTodoReminderNotification,
} from "../../../../../hooks/usePushNotifications";
...
export default function WeeklyCalendarScreen() {
  ...
  const [recommendedActs, setRecommendedActs] = useState<string[]>([]);
  const [todoToDelete, setTodoToDelete] = useState<TodoToDeleteType>({
    dayIndex: null,
    todoId: null,
  });
  ...
  // Fetch recommendations once on mount / language change
  useEffect(() => {
    const load = async () => {
      try {
        const data = await getPrayerRecommendations(i18n.language);
        setRecommendedActs(data.map((r) => r.recommendation_content));
      } catch (error) {
        console.error("Failed to load prayer recommendations:", error);
      }
    };
    load();
  }, [i18n.language]);
      ...
      <Stack.Screen
        options={{
          ...
          headerRight: () => (
            ...
            >
              <ThemedText style={{ color: Colors.universal.favorite }}>
                {t("showPrayerRecommendation")}
              </ThemedText>
            </TouchableOpacity>
          ),
        }}
